...
def curv_mat_gridcpu(v_prime, v_prime_prime):
    ...
    """
    curvature = (
        np.linalg.norm(np.cross(v_prime, v_prime_prime), axis=-1)
        / np.linalg.norm(v_prime, axis=-1) ** 3
    )
    return curvature


def compute_curv_and_dist_mat_gridcpu(
...
def batched_filter_gridcpu(
    path_matrix, dumped_values, i, dimensions, M, path_filter, current=True
):
    ...
    for n in path_indices:
        ...
        if idx == len(path_matrix) - 2:
            new_data = np.concatenate(
                (path_matrix[:3, n, :], path_matrix[-3:, n, :]), axis=0
            )
        else:
            new_data = np.concatenate(
                (path_matrix[:3, n, :], path_matrix[idx : idx + 3, n, :]), axis=0
            )
        ...
        dumped_values = np.concatenate(
            (dumped_values, new_data), axis=1
