...
from CPET.utils.calculator import make_histograms, construct_distance_matrix, construct_distance_matrix_alt, construct_distance_matrix_alt2, construct_distance_matrix_volume, make_fields

class cluster:
    def __init__(self, options):
        self.cluster_method = options["cluster_method"]
        self.cluster_reload = options["cluster_reload"] if "cluster_reload" in options else False
    ...
    def affinity(self):
        ...
        self.cluster_results.n_clusters_ = len(self.cluster_results.cluster_centers_indices)
    ...
    def hdbscan(self):
        ...
        for percentile_threshold in [99.9999,100]:
            threshold=np.percentile(self.distance_matrix.flatten(),percentile_threshold)
    ...
    def compress(self):
        ...
        temp_dict = dict(zip(sorted(list(self.cluster_results["best_performance"][0].keys())),self.cluster_results["cluster_centers_indices"]))
        for k,v in temp_dict.items():
            ...
            if self.cluster_results.names != None:
                compressed_dictionary[str(i)]["name"] = self.cluster_results["names"][self.cluster_results["cluster_centers_indices"][i]]
        ...
        compressed_dictionary["labels"] = [int(i) for i in self.cluster_results["labels"]]
