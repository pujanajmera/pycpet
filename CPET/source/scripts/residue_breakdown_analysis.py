...
def main():
    ...
    parser.add_argument('-cx', '--curvres', help='Histogram curvature resolution', required=True)
    parser.add_argument('-d', '--distbound', help='Histogram Euclidean distance bounds', required=True)
    parser.add_argument('-dx', '--distres', help='Histogram Euclidean distance resolution', required=True)
    ...
    distance_nbins = int((float(distbound[1]) - float(distbound[0])) / float(args.distres))
    curvature_nbins = int((float(curvbound[1]) - float(curvbound[0])) / float(args.curvres))
    ...
    resid_list = calc_temp.residue_number #List of numbers, only has non-zeroed resids from the options file (e.g. zeroing active site)
    ...
    for i in range(len(resname_list)):
        ...
        if residue_dict == residue_dict_prev or residue_dict['resname'] == 'WAT':
            ...
            ValueError('Error: Residue ' + str(residue_dict) + ' is repeated in the list')
        else:
    ...
    for pdb in pdbs:
        ...
        hist_grid_original = get_hist_grid(hist_original, 
                                           distbound, 
                                           distance_nbins, 
                                           curvbound, 
                                           curvature_nbins)
        ...
        for residue in residues_list_temp:
            ...
            resid = residue['resid']
            indices = [i for i, x in enumerate(resname_temp) if x == resname and resid_temp[i] == resid]
    ...
    for residue in residues_list:
        ...
        chi_squared_avg.append([residue, np.mean(chi_squared_residue), np.std(chi_squared_residue)])
