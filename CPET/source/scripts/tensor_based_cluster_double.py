...
def cluster_analyze(distance_matrix, cluster_results, file_list):
    ...
    for i in range(cluster_results["n_clusters"]):
        ...
        temp_dict["index_center"] = cluster_results["cluster_centers_indices"][
            i
        ]
        temp_dict["name_center"] = file_list[temp_dict["index_center"]]
        temp_dict["percentage"] = (
            float(temp_dict["count"])
            / float(len(cluster_results["labels"]))
            * 100
        )
        cluster_indices = [
            y for y, x in enumerate(cluster_results["labels"]) if x == i
        ]
        ...
        temp_zip = zip(
            ...
            [
                distance_matrix[temp_dict["index_center"]][i]
...
def main():
    #Parse arguments
    parser = argparse.ArgumentParser(description='Two-site tensor-based clustering.\nThis is not a restartable script, and will repeat everything from the beginning')
    parser.add_argument('-d1', '--dir1', help='First directory with fields', required=True)
    parser.add_argument('-d2', '--dir2', help='Second directory with fields', required=True)
    ...
    if rank == None:
        rank1 = determine_rank(full_tensor, tensor_threshold, max_rank) #Time-limiting step (and probably memory)
    else:
        rank1 = rank
    ...
    compressed_dictionary = cluster_analyze(distance_matrix, cluster_results, combined_file_list)
